...
import '/widgets/fondo_tinta.dart'; // ✅ BUG 4 FIX: fondo en pantalla de estudio
...
class _PantallaEstudioState extends State<PantallaEstudio> {
  ...
  void _evaluar(int minutosDeEspera) async {
    if (_hanziActual != null) {
      await DatabaseHelper.instance
          .actualizarProgresoSRS(_hanziActual!['id'], minutosDeEspera);
    }
    _siguienteHanzi();
  }
  ...
  // ✅ BUG 2 FIX: consulta tabla 'oraciones' (donde sí se insertan datos),
  //              no la tabla 'ejemplos' que queda vacía.
  void _mostrarModalEjemplos(String hanziSimp) async {
    final db = await DatabaseHelper.instance.database;
    final listaOraciones = await db.query(
      'oraciones',
      where: 'hanzi_simp = ?',
      whereArgs: [hanziSimp],
    );
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        return ListView.builder(
          itemCount: listaOraciones.length,
          itemBuilder: (context, index) {
            final o = listaOraciones[index];
            return ListTile(
              title: Text(
                "${o['oracion_simp']}  (${o['pinyin']})",
                style:
  ...
  void _auditarTrazo(Size canvasSize) {
    ...
    final List<dynamic> trazoEsperado = medians[_trazoCorrectoActual];
    final double startX = trazoEsperado.first[0].toDouble();
    final double startY = trazoEsperado.first[1].toDouble();
    final double endX = trazoEsperado.last[0].toDouble();
    final double endY = trazoEsperado.last[1].toDouble();
    ...
    final Offset inicioEsperado =
        Offset(offX + startX * scaleX, offY + (1024 - startY) * scaleY);
    final Offset finEsperado =
        Offset(offX + endX * scaleX, offY + (1024 - endY) * scaleY);

    final ultimoTrazo = _trazosUsuario.last;
    if (ultimoTrazo.length < 2) return;

    final Offset inicioDedo =
        Offset(ultimoTrazo.first.dx, ultimoTrazo.first.dy);
    final Offset finDedo = Offset(ultimoTrazo.last.dx, ultimoTrazo.last.dy);

    final double tolerancia = canvasSize.width * 0.25;

    if ((inicioDedo - inicioEsperado).distance <= tolerancia &&
        (finDedo - finEsperado).distance <= tolerancia) {
      // ─── Trazo correcto ───
      ...
      // ✅ BUG 1 FIX: resetear _mostrarExito después de 350ms
  ...
  Widget build(BuildContext context) {
    // ✅ BUG 4 FIX: FondoTintaChina envuelve toda la pantalla de estudio
    return FondoTintaChina(
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.transparent,
          elevation: 0,
          title: Text('Estudiando HSK ${widget.nivelHSK}',
        ...
        body: _hanziActual == null
            ...
            : Column(
                children: [
                  // ── Panel superior: pinyin, audio, ejemplos ──
                  Expanded(
                    flex: 2,
                    child: Container(
                          ...
                          Padding(
                            ...
                            child: ClipRRect(
                              ...
                              child: BackdropFilter(
                                ...
                                child: InkWell(
                                  // ✅ BUG 2 FIX: pasa simplificado, no id
                              ...
                              builder: (context, constraints) {
                                ...
                                final List<String> vectores =
                                    _hanziActual!['trazos'] != null
                                        ? List<String>.from(
                                            jsonDecode(_hanziActual!['trazos']))
                                        : [];
                                return Stack(
                                  children: [
                                    Positioned.fill(
                            ...
                            ? Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    _botonSRS(
                                        'Medio', Colors.orange, 3 * 24 * 60),
                                    _botonSRS(
                                        'Fácil', Colors.green, 14 * 24 * 60),
                                  ],
                                ),
                              )
